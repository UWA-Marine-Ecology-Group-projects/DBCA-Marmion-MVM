...
server = function(input, output, session) {
  ...
  selected.data <- reactive({
    ...
    checkboxall <-
      c(
        ...
        unique(pressures.acc$checkbox)
      ) 
    
    # create a dataframe with all the activity variables
    all.acc <- bind_rows(activity.acc, values.acc, other.acc, pressures.acc)
  ...
  observeEvent(input$nextspatial, {
    output$activity_plots <- renderUI({
      activities.selected <- selected.data() %>%
        filter(!category %in% c("local_knowledge", "pressures_and_threats"))
  ...
  # Create plot tag list and create leaflet outputs for each pressure selected ----
  observeEvent(input$nextspatial, {
    output$pressures_plots <- renderUI({
      activities.selected <- selected.data() %>%
  ...
  observeEvent(input$nextactivities, {
    ...
    observeEvent(input$submit, {
      ...
      sheet_append("15CoscrgN2DDG00LgLNqWQMcU_U8X2r9eyAD5gP7eTXY", sheet = 1, data = data.gs)
      
      # append to googledrive ----
      clicks.gs <- clicks %>% as.list() %>% data.frame()
      sheet_append("15CoscrgN2DDG00LgLNqWQMcU_U8X2r9eyAD5gP7eTXY", sheet = 2, data = clicks.gs)
